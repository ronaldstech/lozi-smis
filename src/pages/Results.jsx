  ...
  <Paper
    ...
    sx={{
      ...
      "&:hover": {
        ...
        boxShadow: "0 9px 20px rgba(0,0,0,.08)",
      },
    }}
  >
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const studentRes = await fetch(
        `${API_URL}?getStudents=true&school_type=${schoolType}`,
      );
      ...
      const staffRes = await fetch(
        `${API_URL}?getStaffA=${academic.id}&school_type=${schoolType}`,
      );
      ...
      const avgProgress =
        allProgress.length > 0 ? Math.round(totalProg / allProgress.length) : 0;
        ...
        >
          ...
          >
            <Box>
              ...
              >
                <SchoolIcon size={16} />
                Best 6 subject compiler & report generator for{" "}
                {academic?.name || "current session"}.
              </Typography>
            </Box>
              ...
              <Tooltip title="Refresh Session Status">
                ...
                >
                  {loading ? (
                    ...
                    <ProgressIcon size={16} style={{ marginRight: 8 }} />
                  )}
                  Sync
                </Button>
              </Tooltip>
        ...
        >
          ...
          <Grid item xs={12} sm={6} md={3}>
            <StatCard
              ...
              color="#f59e0b"
              subtitle={
                isSenior ? "Senior Secondary Logic" : "Junior Secondary Logic"
              }
              loading={loading}
            />
          </Grid>
                ...
                <Stack spacing={3}>
                  ...
                  {stats.avgProgress < 100 && (
                    <Alert severity="warning" sx={{ borderRadius: 2 }}>
                      ...
                      Overall grading is at {stats.avgProgress}%. Generating
                      reports now may result in incomplete Best 6 calculations.
                    </Alert>
                  )}
                  ...
                  >
                    ...
                    >
                      <InfoIcon size={14} />
                      {isSenior
                        ? "Senior logic active: Aggregating 5 best grades (1-9) + Compulsory English."
                        : isJunior
                          ? "Junior logic active: Aggregating 5 best marks (0-100) + Compulsory English."
                  ...
                  <Button
                    ...
                    disabled={
                      !formData.form_select ||
                      !formData.type_select ||
                      processing
                    }
                    sx={{
                      borderRadius: 4,
            ...
            <Stack spacing={3}>
              ...
              >
                ...
                <Stack spacing={2}>
                  {[
                    {
                      title: "English Constraint",
                      desc: "English (Subject ID 12) is used in all Best 6 rankings regardless of score.",
                    },
                    {
                      title: "Optimization",
                      desc: "System automatically picks the top 5 remaining subjects for each student.",
                    },
                    {
                      title: "Persistence",
                      desc: "Totals and PASS/FAIL status are permanently stored in the total_marks table.",
                    },
                  ].map((rule, idx) => (
                    <Box key={idx}>
                      <Typography
                        variant="body2"
                        sx={{ fontWeight: 700, color: "#475569", fontSize: 12 }}
                      >
                        {rule.title}
                      </Typography>
                      <Typography variant="caption" color="textSecondary">
              ...
              >
                ...
                <Typography variant="body2" sx={{ opacity: 0.8, mb: 2 }}>
                  If you notice errors in the calculations, please verify that
                  subject assignments and grades are correctly entered in the
                  Workload tab.
                </Typography>
                <Button
                  sx={{
