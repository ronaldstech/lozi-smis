...
import { TaskSquare, Eye } from "iconsax-react";
...
function StaffHome() {
  ...
  const progress =
    rows.length > 0 ? Math.round((gradedCount / rows.length) * 100) : 0;
        ...
        subjects.map(async (subject) => {
          try {
            ...
            const studentRes = await fetch(
              `${API_URL}?${studentParams.toString()}`,
            );
  ...
  const deleteMarkRow = async (id, subject, form, academic_id) => {
    if (
      !window.confirm(
        "Are you sure you want to delete ALL marks for this student record? This cannot be undone.",
      )
    )
      return;
    ...
    try {
      ...
      if (res.status) {
        ...
        setRows((prev) =>
          prev.map((row) => (row.id === id ? { ...row, end_term: "" } : row)),
        );
      } else {
        Toastify({ text: res.message, backgroundColor: "#ef4444" }).showToast();
      }
        ...
        >
          <Box>
            ...
            >
              ...
              >
                <TaskSquare variant="Bulk" size={24} color="#ff0000" />
              </Box>
              <Typography
                variant="h5"
            ...
            <Typography
              variant="body2"
              sx={{ color: "#64748b", ml: { md: 6 } }}
            >
              Manage your assigned subjects and student marks for{" "}
              {academic.name} — Term {academic.term}.
            </Typography>
          </Box>

          <Chip
            icon={<Eye size={16} variant="Bold" color="#5fc568" />}
            label={
              info.status === "active"
                ? "Grading Window Open"
